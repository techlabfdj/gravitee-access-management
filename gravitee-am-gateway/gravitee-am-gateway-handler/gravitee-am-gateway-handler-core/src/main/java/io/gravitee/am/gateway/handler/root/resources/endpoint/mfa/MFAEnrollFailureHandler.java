...
public class MFAEnrollFailureHandler extends AbstractErrorHandler {
    ...
    private void handleException(RoutingContext context, Throwable throwable) {
        String errorDescription = throwable == null ? "MFA Enrollment failed for unexpected reason" : throwable.getMessage();
        final MultiMap queryParams = updateQueryParams(context, errorDescription);
        String errorPageURL = UriBuilderRequest.resolveProxyRequest(context.request(), context.get(CONTEXT_PATH) + errorPage, queryParams, true);
        String toHash = MFA_ENROLL_VALIDATION_FAILED + "$" + errorDescription;
        context.session().put(ERROR_HASH, HashUtil.generateSHA256(toHash));
        doRedirect(context.response(), errorPageURL);
    }
