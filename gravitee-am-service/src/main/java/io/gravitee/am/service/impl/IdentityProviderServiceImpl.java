...
public class IdentityProviderServiceImpl implements IdentityProviderService {
    ...
    @Override
    public Single<IdentityProvider> updatePasswordPolicy(String domain, String id, AssignPasswordPolicyToIdp assignPasswordPolicyToIdp) {
        LOGGER.debug("Assigning Password Policy {} to IdentityProvider {} for domain {}", assignPasswordPolicyToIdp.getPasswordPolicy(), id, domain);
        ...
        return identityProviderRepository.findById(ReferenceType.DOMAIN, domain, id)
                ...
                .flatMap(oldIdentity -> {
                    IdentityProvider identityToUpdate = new IdentityProvider(oldIdentity);
                    identityToUpdate.setUpdatedAt(new Date());
                    identityToUpdate.setPasswordPolicy(assignPasswordPolicyToIdp.getPasswordPolicy());
