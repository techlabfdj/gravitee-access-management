...
import io.gravitee.am.certificate.api.CertificateBundle;
...
class WebClientOptionsConfigurer {
    ...
    void setMTLSSettings(WebClientOptions options,
                         CertificateProvider clientCertificate) {
        JksOptions keyStoreOptions = new JksOptions();
        CertificateBundle clientsCertificate = clientCertificate.getCertificateBundleDetails()
                .switchIfEmpty(MaybeNever.error(() -> new IllegalStateException("Couldn't load the certificate")))
                .blockingGet();
        keyStoreOptions
                .setValue(Buffer.buffer(clientsCertificate.value()))
                .setAlias(clientsCertificate.alias())
                .setPassword(clientsCertificate.storepass())
                .setAliasPassword(clientsCertificate.keypass());
