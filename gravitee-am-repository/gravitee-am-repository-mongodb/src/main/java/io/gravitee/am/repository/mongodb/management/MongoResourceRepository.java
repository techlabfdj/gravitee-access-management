...
public class MongoResourceRepository extends AbstractManagementMongoRepository implements ResourceRepository {
    ...
    public Single<Page<Resource>> findByDomainAndClient(String domain, String client, int page, int size) {
        ...
        Single<List<Resource>> resourcesOperation = Observable.fromPublisher(withMaxTime(resourceCollection.find(and(eq(FIELD_DOMAIN, domain), eq(FIELD_CLIENT_ID, client)))).sort(new BasicDBObject(FIELD_UPDATED_AT, -1)).skip(size * page).limit(size)).map(this::convert).collect(Collectors.toList());
